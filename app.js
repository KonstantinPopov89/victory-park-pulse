(() => {
  ...
  // Our own HTTPS origin on the Paris host. Same ciphertext as the Pages copy,
  // read only when GitHub cannot be reached, so an outage stops freezing the numbers.
  const FALLBACK_DATA_URL = "https://mtg-par-01.46-8-224-28.sslip.io:8444/data.enc";
  const STALE_PROBE_MINUTES = 30;
  const LANGUAGE_KEY = "vpp_lang";
  const REMEMBERED_ACCESS_KEY = "vpp_remembered_access";
  const TIMEZONE = "Asia/Yerevan";
  const AUTO_REFRESH_MS = 60 * 1000;
  const REQUEST_TIMEOUT_MS = 30 * 1000;
  const REFRESH_RETRY_DELAYS_MS = [5 * 1000, 15 * 1000];
  ...
  const I18N = {
    ru: {
      ...
      sourceCorrectionDelayed: "Текущие данные сверены · историческая коррекция задержана",
      ...
      attractionsTailStale: "Историческая сверка аттракционов отложена; текущие списания обновлены.",
      ...
      footerSchedule: "Источник: каждые 15 минут · проверка страницы: каждую минуту · ночная пауза источника 01:00–08:00 по Еревану",
      footerSnapshot: "данные по {date}",
      footerBrowserCheck: "проверка публикации {date}",
      fresh: "Свежие данные",
      delayed: "Данные задерживаются",
      offline: "Нет связи",
      night: "Ночная пауза",
      morning: "Утреннее обновление",
      ...
      morningDetail: "Ожидается утренний выпуск до 08:15 · снимок {age}",
      errorDetail: "Время данных отсутствует или некорректно",
      justNow: "только что",
    ...
    hy: {
      ...
      sourceCorrectionDelayed: "Ընթացիկ տվյալները համադրված են · պատմական ճշգրտումը ուշանում է",
      ...
      attractionsTailStale: "Ատրակցիոնների պատմական ստուգումը հետաձգվել է․ ընթացիկ գանձումները թարմացված են։",
      ...
      footerSchedule: "Աղբյուրը՝ 15 րոպեն մեկ · էջի ստուգումը՝ ամեն րոպե · աղբյուրի գիշերային դադար՝ 01:00–08:00 Երևանի ժամանակով",
      footerSnapshot: "տվյալները՝ {date} դրությամբ",
      footerBrowserCheck: "հրապարակումը ստուգվել է՝ {date}",
      fresh: "Թարմ տվյալներ",
      delayed: "Տվյալներն ուշանում են",
      offline: "Ցանցից դուրս",
      night: "Գիշերային դադար",
      morning: "Առավոտյան թարմացում",
      ...
      morningDetail: "Առավոտյան թարմացումը սպասվում է մինչև 08:15 · պատկերը {age}",
      errorDetail: "Տվյալների ժամանակը բացակայում է կամ սխալ է",
      justNow: "հենց նոր",
  ...
  function relativeAge(value) {
    ...
    if (minutes < 60) return tr("minutesAgo", { count: Math.floor(minutes) });
  ...
  function yerevanClock() {
    ...
    const midnight = now - (minuteOfDay * 60 + part("second")) * 1000 - (now % 1000);
    return { minuteOfDay, midnight };
  }
  ...
  function validateAttractions(attractions) {
    ...
    const localDate = parseDateOnly(cutoff.local_date);
    const fullRefreshDate = parseDateOnly(cache?.full_refresh_date);
    const correctionTailAge = localDate && fullRefreshDate
      ? Math.round((localDate.getTime() - fullRefreshDate.getTime()) / 86_400_000)
      : null;
    if (
      !cache ||
      cache.range_start !== weeks.at(-1).start ||
      cache.range_end !== cutoff.local_date ||
      !Number.isFinite(Date.parse(cache.updated_at)) ||
      !Number.isInteger(cache.correction_tail_age_days) ||
      cache.correction_tail_age_days !== correctionTailAge ||
      cache.correction_tail_age_days < 0 ||
      cache.correction_tail_age_days > 7 ||
      typeof cache.correction_tail_stale !== "boolean" ||
      cache.correction_tail_stale !== (cache.correction_tail_age_days > 0)
    ) throw new AppError("invalid");
  }
  ...
  function validatePayload(payload) {
    ...
    const generatedAt = Date.parse(payload.generated);
    if (!Number.isFinite(generatedAt)) throw new AppError("invalid");
    if (
      park.id !== "victory" ||
      park.currency !== "AMD" ||
    ...
    const correction = park.source_quality?.historical_correction;
    if (
      !correction ||
      correction.max_age_hours !== 30 ||
      typeof correction.stale !== "boolean"
    ) throw new AppError("invalid");
    if (correction.last_refreshed_at === null) {
      if (correction.stale !== true) throw new AppError("invalid");
    } else {
      const refreshedAt = Date.parse(correction.last_refreshed_at);
      if (!Number.isFinite(refreshedAt)) throw new AppError("invalid");
      const ageMs = generatedAt - refreshedAt;
      const maxAgeMs = correction.max_age_hours * 60 * 60 * 1000;
      const toleranceMs = 5 * 60 * 1000;
      if (ageMs < -toleranceMs) throw new AppError("invalid");
      if (
        correction.stale !== (ageMs > maxAgeMs) &&
        Math.abs(ageMs - maxAgeMs) > toleranceMs
      ) throw new AppError("invalid");
    }
  ...
  async function fetchPayload(candidate, dataUrl = DATA_URL) {
    ...
    try {
      let response;
      try {
        response = await fetch(`${dataUrl}?t=${Date.now()}`, {
      ...
      if (!response.ok) {
        ...
        throw new AppError(transient ? "network" : "invalid");
      }
      try {
        envelope = await response.json();
      } catch (_error) {
        throw new AppError(controller.signal.aborted ? "network" : "invalid");
      }
    } finally {
  ...
  function newerPayload(primary, backup) {
    const first = Date.parse(primary?.park?.data_as_of);
    const second = Date.parse(backup?.park?.data_as_of);
    if (!Number.isFinite(second)) return primary;
    if (!Number.isFinite(first)) return backup;
    if (second !== first) return second > first ? backup : primary;
    // Same cutoff: the later build wins, matching the refresh ordering.
    return Date.parse(backup.generated) > Date.parse(primary.generated) ? backup : primary;
  }

  function worthProbingSecondOrigin(payload) {
    // Publication to Pages fails far more often than the network does. A
    // reachable but old primary is exactly that case, and our own origin is
    // written before the push, so it can already hold the newer snapshot.
    const { minuteOfDay } = yerevanClock();
    if (minuteOfDay >= 60 && minuteOfDay < 8 * 60 + 15) return false;
    const age = ageMinutes(payload?.park?.data_as_of);
    return age === null || age > STALE_PROBE_MINUTES;
  }

  async function fetchPayloadResilient(candidate) {
    let primary;
    try {
      primary = await fetchPayload(candidate);
    } catch (error) {
      // Only an unreachable primary earns a second origin. A wrong password,
      // an invalid envelope or a broken payload must still surface as itself.
      if (!(error instanceof AppError) || error.code !== "network") throw error;
      return await fetchPayload(candidate, FALLBACK_DATA_URL);
    }
    if (!worthProbingSecondOrigin(primary)) return primary;
    try {
      return newerPayload(primary, await fetchPayload(candidate, FALLBACK_DATA_URL));
    } catch (_error) {
      // The backup adds nothing here; never let it hide a usable primary.
      return primary;
    }
  }
  ...
  function renderHero(park) {
    ...
    const correctionDelayed = park.source_quality?.historical_correction?.stale === true;
    byId("sourceQuality").textContent = tr(
      !reconciled
        ? "sourceUnverified"
        : correctionDelayed
          ? "sourceCorrectionDelayed"
          : "sourceReconciled"
    );
  ...
  function renderAttractions(attractions, fallbackCutoffMinutes) {
    ...
    const tailStale = quality.cache?.correction_tail_stale === true;
    const qualityNode = byId("attractionsQuality");
    qualityNode.textContent = tr(
      unmapped > 0
        ? "attractionsUnmapped"
        : hasDrift
          ? "attractionsMappingDrift"
          : tailStale
            ? "attractionsTailStale"
            : "attractionsUnverified"
    );
    qualityNode.classList.toggle("is-hidden", reconciled && !hasDrift && !tailStale);
  }
  ...
  function statusState() {
    ...
    const cutoff = Date.parse(park.data_as_of);
    if (!Number.isFinite(cutoff) || cutoff > Date.now()) return "error";
    ...
    const { minuteOfDay, midnight } = yerevanClock();
    // The final expected 00:45 source run may lag by one 15-minute slot.
    const hasCurrentNightSnapshot = cutoff >= midnight + 30 * 60_000;
    if (minuteOfDay >= 60 && minuteOfDay < 480) return hasCurrentNightSnapshot ? "night" : "delayed";
    // Allow the first morning publication to finish, without hiding a new 08:00 snapshot.
    if (minuteOfDay >= 480 && minuteOfDay < 495 && cutoff < midnight + 480 * 60_000) {
      return hasCurrentNightSnapshot ? "morning" : "delayed";
    }
  ...
  function renderStatus() {
    ...
    const labels = { fresh: "fresh", delayed: "delayed", offline: "offline", night: "night", morning: "morning", error: "dataError" };
    ...
    if (status === "morning") detailKey = "morningDetail";
    byId("statusStrip").dataset.status = status === "morning" ? "night" : status;
    byId("statusLabel").textContent = tr(labels[status]);
    byId("statusDetail").textContent = tr(detailKey, { age });
    byId("footerBrowserCheck").textContent = tr("footerBrowserCheck", {
      date: lastBrowserCheckAt === null ? tr("noData") : formatSnapshot(lastBrowserCheckAt)
    });
  }
  ...
  function showDashboard(payload, candidate) {
    ...
    lastBrowserCheckAt = new Date(Date.now()).toISOString();
  ...
  async function unlockCandidate(candidate, { remember, automatic }) {
    ...
    setUnlockBusy(true);
    try {
      const payload = await fetchPayloadResilient(candidate);
  ...
  function clearRefreshRetry() {
    if (refreshRetryTimer !== null) window.clearTimeout(refreshRetryTimer);
    refreshRetryTimer = null;
  }

  function scheduleRefreshRetry() {
    if (refreshRetryTimer !== null || !passphrase || document.hidden || !navigator.onLine) return;
    if (refreshRetryCount >= REFRESH_RETRY_DELAYS_MS.length) return;
    const delay = REFRESH_RETRY_DELAYS_MS[refreshRetryCount++];
    refreshRetryTimer = window.setTimeout(() => {
      refreshRetryTimer = null;
      void refreshData({ retry: true });
    }, delay);
  }

  async function refreshData({ retry = false } = {}) {
    if (!passphrase || document.hidden || !navigator.onLine) return;
    if (refreshInFlight) {
      refreshQueued = true;
      return;
    }
    clearRefreshRetry();
    if (!retry) refreshRetryCount = 0;
    refreshInFlight = true;
    const activePassphrase = passphrase;
    setRefreshBusy(true);
    try {
      const payload = await fetchPayloadResilient(activePassphrase);
      if (passphrase !== activePassphrase) return;
      if (currentPayload) {
        const incomingCutoff = Date.parse(payload.park.data_as_of);
        const previousCutoff = Date.parse(currentPayload.park.data_as_of);
        if (incomingCutoff < previousCutoff || (incomingCutoff === previousCutoff && Date.parse(payload.generated) < Date.parse(currentPayload.generated))) {
          throw new AppError("invalid");
        }
      }
      currentPayload = payload;
      ...
      if (passphrase !== activePassphrase) return;
      refreshFailed = true;
      refreshFailureCode = error instanceof AppError ? error.code : "invalid";
      ...
      refreshQueued = false;
      const transientFailure = refreshFailed && refreshFailureCode === "network";
      const canFollowUp = !transientFailure || refreshRetryCount < REFRESH_RETRY_DELAYS_MS.length;
      if (queued && canFollowUp && passphrase && !document.hidden && navigator.onLine) {
        // A burst of resume events produces one follow-up, never parallel requests.
        // A recovery attempt also consumes the bounded transient retry budget.
        if (transientFailure) refreshRetryCount++;
        void refreshData({ retry: true });
  ...
  function startAutoRefresh() {
    ...
    refreshTimer = window.setInterval(() => {
      if (document.hidden || !currentPayload) return;
      // Age advances even while a request is slow, offline, or showing the same payload.
      renderStatus();
      void refreshData();
    }, AUTO_REFRESH_MS);
  }
  ...
  function bindEvents() {
    ...
    const resumeRefresh = () => {
      if (!document.hidden && currentPayload) {
        renderStatus();
        void refreshData();
      }
    };
